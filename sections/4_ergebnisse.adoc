...
Dabei zeigte sich das, sich das Metadatenfeld `PNG:SignicantBits` bei von MobiStego veränderten Bildern
von `8 8 8` zu `8 8 8 8` änderte.
Eine zweite Änderung konnte beim Feld `PNG:ColorType` festgestellt werden,
welche sich bei Bildern von MobiStego von `2` auf `6` und bei Bildern von Pictograph von `0` auf `2` beläuft.

Somit lassen sich diese Funde als Regeln zur Erkennung von MobiStego und Pictograph einsetzen.
...
image::eval-detect_bool_mobistego.png[pdfwidth=80%]

Es zeigt sich, dass mehr Stego-Bilder falsch MobiStego zugeordnet wurden als korrekte.
Die Rate der korrekt erkannten Negativen ist dagegen mit 9899 zu 309 sehr gut.

image::eval-detect_bool_passlok.png[pdfwidth=80%]

Die meisten Bilder wurden korrekt Passlok zugeteilt oder als andere Stego-App identifiziert.
Bemerkenswert ist, dass es dabei keine falsche Negativ-Zuordnung gab.

image::eval-detect_bool_pictograph.png[pdfwidth=80%]

Für Pictograph wurden beinahe ebenso viele Pictograph falsch wie richtig zugeordnet.

image::eval-detect_bool_pixelknot.png[pdfwidth=80%]

Auch im Fall von PixelKnot wurde beinahe 1500 Bilder falsch dieser App zugeordnet.
Die Anzahl an fälschlich als nicht von Passlok erstellte Bilder ist allerdings sehr niedrig.

image::eval-detect_bool_pocketstego.png[pdfwidth=80%]

PocketStego weist sehr ähnliche Ergebnisse wie PixelKnot auf.

image::eval-detect_bool_steganographym.png[pdfwidth=80%]
...
Ebenso ist die Anzahl an Falschnegativen sehr hoch.

<<<

Die folgende Tabelle stellt die Ergebnisse mit den Metriken der Genauigkeit (_Accuracy_),
Präzision (_Precision_), Sensitivität (_Recall_) und dem F1-Score dar.
...
Unter der Bedingung, dass eine Detektion immer gewertet wird,
weisen SteganographyM und MobiStego mit 59,55 % un 68,12 % die schlechteste Genauigkeit auf,
während Passlok, PixelKnot und PocketStego mit 98,86 % und 91,18 % und 90,36 % am besten abschneiden.

Die Präzision ist bei allen Stego-Apps mit Ausnahme von Passlok und StegnographyM relativ niedrig,
sodass eine positive Erkennung nur mit einer Wahrscheinlichkeit um die 50 % richtig ist.
Bei Passlok dagegen wird jedes Bild korrekt der App zugeordnet,
während bei SteganographyM mit 19,67 % nur jedes fünfte Bild richtigerweise der App zugewiesen wird.
